...
\begin{frame}{Parameters services}
  ...
  \end{itemize}
  Try to inspect them with \texttt{ros2 service type}! They all take \textbg{groups of parameters} on which to operate, and can be called either from a \textbg{CLI tool} or from a \textbg{client node}: that is the way to go if you want to \textbg{automate parameter updates}.
\end{frame}

% --- Coding with parameters ---
\begin{frame}{Coding with parameters}{Hints and best practices}
	\begin{itemize}
		\item Parameters are referred to by their \textbg{name}.
		\item Before being used, a parameter must be \textbg{declared} to the middleware: this is usually done in the constructor of a node specifying their \textbg{name} and \textbg{default value}.
		\item Parameter values can be retrieved \textbg{atomically} by calling an API, but accessing the middleware's internals to do this might be \textbg{slow}: define \textbg{class member variables} that track the value of each parameter by being updated each time the parameter is.
	\end{itemize}
\end{frame}
\begin{frame}{Coding with parameters}{Suggested TODO list}
  \begin{enumerate}
    ...
    \item Define a \textbg{callback} to be called when a parameter is updated.
